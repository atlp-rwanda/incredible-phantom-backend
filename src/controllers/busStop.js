...
const createBusStop = async (req,res) => {
    try{
        ...
        if(searchBusStop.length === 0 && searchBusStopId.length === 0){
            ...
            return succesRes(res,200,"Bus stop created successfully" , Stop);
        } else{
            return succesRes(res,404,"Bus stop already exists");
        }
    } catch (error) {
...
const oneStop = async (req,res) => {
    try{
        ...
        if(!Stop){
            return succesRes(res,404,"bus stop not found :(");
        }else{
...
const updateBusStop = async (req,res) => {
    try{
        ...
        if (searchBusStop) {
            ...
            return succesRes(res,404,"busStop not found");
        }
    } catch (error) {
            return errorRes(res,500,"Internal server error : " + error.message);
    }
