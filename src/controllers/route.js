...
const createRoute = async (req, res) => {
  try {
    const { origin, destination, distance, routeID } = req.body;
    ...
    const searchRouteId = await Route.findAll({ where: { routeID: routeId } });
...
const addBusStop = async (req, res) => {
  try {
    ...
    if (searchingBusStop && searchingRoute) {
      const existence = await Route.findOne({
        where: {
          ...
          busStops: { [Op.contains]: [searchingBusStop.busStopId] },
        },
      });
      if (!existence) {
    ...
    return errorRes(
      res,
      404,
      res.__('Neither bus stop ID or route ID does not exists')
    );
  } catch (error) {
    return errorRes(
      res,
