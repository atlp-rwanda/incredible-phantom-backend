...
const createRoute = async (req,res) => {
    try{
        ...
        const searchRoute = await Route.findAll( { where : { origin : req.body.origin , destination : req.body.destination } } );
    ...
    try{
        ...
        if (searchRoute) {
            ...
            if(existence.length === 0){
                ...
                return succesRes(res,404,"One of the routes has the same information");
            }
        }else{
            return succesRes(res,404,"Route not found");
        }
...
const addBusStop = async (req,res) => {
    try{
      ...
      if(searchingBusStop && searchingRoute){
        ...
        return succesRes(res,404,"There is no route corresponding to that ID");
      }else if(searchingRoute == null){
          return succesRes(res,404,"There is no matching bus stops");
      }else{
